...
async def test_anticipatory_memory(test_engine):
    ...
    node_a = await test_engine.store(
        "I love learning about Machine Learning and AI algorithms."
    )
    node_b = await test_engine.store(
        "Neural networks and deep learning models are very powerful."
    )
    ...
    results = await test_engine.query(
        "I love learning about Machine Learning and AI algorithms.", top_k=2
    )
    ...
    assert (
        node_b in test_engine.tier_manager.hot
    ), "Anticipatory engine failed to preload node_b."
