import pytest
import numpy as np
from mnemocore.core.synapse import SynapticConnection
from mnemocore.core.synapse_index import SynapseIndex

@pytest.mark.asyncio
async def test_get_multi_hop_neighbors():
    index = SynapseIndex()
    
    # Create nodes A, B, C, D
    # A <-> B (weight 0.8)
    # B <-> C (weight 0.5)
    # C <-> D (weight 0.9)
    # A <-> C (weight 0.1)
    
    syn_ab = await index.add_or_fire("A", "B")
    syn_ab.strength = 0.8
    
    syn_bc = await index.add_or_fire("B", "C")
    syn_bc.strength = 0.5
    
    syn_cd = await index.add_or_fire("C", "D")
    syn_cd.strength = 0.9
    
    syn_ac = await index.add_or_fire("A", "C")
    syn_ac.strength = 0.1
    
    # 1 hop from A
    hops_1 = await index.get_multi_hop_neighbors("A", depth=1)
    assert hops_1["B"] == pytest.approx(0.8)
    assert hops_1["C"] == pytest.approx(0.1)
    assert "D" not in hops_1
    
    hops_2 = await index.get_multi_hop_neighbors("A", depth=2)
    assert hops_2["B"] == pytest.approx(0.8)
    assert hops_2["C"] == pytest.approx(0.4)
    assert hops_2["D"] == pytest.approx(0.09)
    
    hops_3 = await index.get_multi_hop_neighbors("A", depth=3)
    assert hops_3["D"] == pytest.approx(0.36)
