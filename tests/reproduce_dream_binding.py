...
from mnemocore.core.config import HAIMConfig, SubconsciousAIConfig, get_config
from mnemocore.core.engine import HAIMEngine
from mnemocore.core.node import MemoryNode
from mnemocore.core.subconscious_ai import ModelClient, SubconsciousAIWorker
...
async def run_verification():
    ...
    cfg = dataclasses.replace(cfg, paths=new_paths, subconscious_ai=sub_cfg)
    ...
    mock_response = {"bridges": {"1": ["feline_concept"]}}

    mock_client = MockModelClient({"dreaming": json.dumps(mock_response)})
    # Inject mock client
    ...
    if synapse:
        logger.success(
            f"SUCCESS: Synapse found between {weak_node.id} and {related_node.id}"
        )
        logger.info(f"Synapse Strength: {synapse.strength}")
    else:
        logger.error(
            f"FAILURE: No synapse found between {weak_node.id} and {related_node.id}"
        )
    ...
    if os.path.exists(cfg.paths.synapses_file):
        with open(cfg.paths.synapses_file, "r") as f:
            ...
            if weak_node.id in content and related_node.id in content:
                ...
                logger.warning(
                    "WARNING: Synapse file exists but IDs not found (might require a save trigger)."
                )
                ...
                with open(cfg.paths.synapses_file, "r") as f2:
                    ...
                    if weak_node.id in content2:
                        logger.success(
                            "SUCCESS: Synapse persisted after explicit save."
                        )
                    else:
