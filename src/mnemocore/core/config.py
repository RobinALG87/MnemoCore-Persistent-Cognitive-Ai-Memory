...
class SecurityConfig:
    ...
    cors_origins: list[str] = field(
        default_factory=lambda: ["http://localhost:3000", "http://localhost:8100"]
    )
...
class SubconsciousAIConfig:
    ...
    # Model configuration
    model_provider: str = (
        "ollama"  # "ollama" | "lm_studio" | "openai_api" | "anthropic_api"
    )
    ...
    micro_self_improvement_enabled: bool = (
        False  # Pattern analysis (disabled by default)
    )
...
class WebhookConfig:
    ...
    default_headers: dict = field(
        default_factory=lambda: {"User-Agent": "MnemoCore-Webhook/1.0"}
    )
...
class HAIMConfig:
    ...
    attention_masking: AttentionMaskingConfig = field(
        default_factory=AttentionMaskingConfig
    )
    ...
    embedding_registry: EmbeddingRegistryConfig = field(
        default_factory=EmbeddingRegistryConfig
    )
    search: SearchConfig = field(default_factory=SearchConfig)
    dreaming: DreamingConfig = field(default_factory=DreamingConfig)
    vector_compression: VectorCompressionConfig = field(
        default_factory=VectorCompressionConfig
    )
    ...
    memory_scheduler: MemorySchedulerConfig = field(
        default_factory=MemorySchedulerConfig
    )
...
def _build_tier(name: str, raw: dict) -> TierConfig:
    ...
    return TierConfig(
        max_memories=_env_override(
            f"{prefix}_MAX_MEMORIES", raw.get("max_memories", 0)
        ),
        ltp_threshold_min=_env_override(
            f"{prefix}_LTP_THRESHOLD_MIN", raw.get("ltp_threshold_min", 0.0)
        ),
...
def load_config(path: Optional[Path] = None) -> HAIMConfig:
    ...
    dimensionality = _env_override("DIMENSIONALITY", raw.get("dimensionality", 16384))
    ...
    qdrant = QdrantConfig(
        url=_env_override("QDRANT_URL", qdrant_raw.get("url", "http://localhost:6333")),
    ...
    security = SecurityConfig(
        ...
        rate_limit_enabled=_env_override(
            "RATE_LIMIT_ENABLED", sec_raw.get("rate_limit_enabled", True)
        ),
        rate_limit_requests=_env_override(
            "RATE_LIMIT_REQUESTS", sec_raw.get("rate_limit_requests", 100)
        ),
        rate_limit_window=_env_override(
            "RATE_LIMIT_WINDOW", sec_raw.get("rate_limit_window", 60)
        ),
        trusted_proxies=trusted_proxies,
        hsts_enabled=_env_override("HSTS_ENABLED", sec_raw.get("hsts_enabled", True)),
        public_health_enabled=_env_override(
            "PUBLIC_HEALTH_ENABLED", sec_raw.get("public_health_enabled", True)
        ),
        public_stats_enabled=_env_override(
            "PUBLIC_STATS_ENABLED", sec_raw.get("public_stats_enabled", False)
        ),
        public_rate_limits_enabled=_env_override(
            "PUBLIC_RATE_LIMITS_ENABLED",
            sec_raw.get("public_rate_limits_enabled", False),
        ),
        public_metrics_enabled=_env_override(
            "PUBLIC_METRICS_ENABLED", sec_raw.get("public_metrics_enabled", False)
        ),
    ...
    mcp = MCPConfig(
        ...
        api_base_url=_env_override(
            "MCP_API_BASE_URL", mcp_raw.get("api_base_url", "http://localhost:8100")
        ),
        api_key=_env_override(
            "MCP_API_KEY", mcp_raw.get("api_key", sec_raw.get("api_key"))
        ),
        timeout_seconds=_env_override(
            "MCP_TIMEOUT_SECONDS", mcp_raw.get("timeout_seconds", 15)
        ),
    ...
    hysteresis = HysteresisConfig(
        promote_delta=_env_override(
            "HYSTERESIS_PROMOTE_DELTA", hyst_raw.get("promote_delta", 0.15)
        ),
        demote_delta=_env_override(
            "HYSTERESIS_DEMOTE_DELTA", hyst_raw.get("demote_delta", 0.10)
        ),
    )
    ...
    ltp = LTPConfig(
        initial_importance=_env_override(
            "LTP_INITIAL_IMPORTANCE", ltp_raw.get("initial_importance", 0.5)
        ),
        decay_lambda=_env_override(
            "LTP_DECAY_LAMBDA", ltp_raw.get("decay_lambda", 0.01)
        ),
        permanence_threshold=_env_override(
            "LTP_PERMANENCE_THRESHOLD", ltp_raw.get("permanence_threshold", 0.95)
        ),
        half_life_days=_env_override(
            "LTP_HALF_LIFE_DAYS", ltp_raw.get("half_life_days", 30.0)
        ),
    )
    ...
    attention_masking = AttentionMaskingConfig(
        enabled=_env_override(
            "ATTENTION_MASKING_ENABLED", attn_raw.get("enabled", True)
        ),
    )
    ...
    consolidation = ConsolidationConfig(
        ...
        interval_seconds=_env_override(
            "CONSOLIDATION_INTERVAL_SECONDS", cons_raw.get("interval_seconds", 3600)
        ),
        similarity_threshold=_env_override(
            "CONSOLIDATION_SIMILARITY_THRESHOLD",
            cons_raw.get("similarity_threshold", 0.85),
        ),
        min_cluster_size=_env_override(
            "CONSOLIDATION_MIN_CLUSTER_SIZE", cons_raw.get("min_cluster_size", 2)
        ),
        hot_tier_enabled=_env_override(
            "CONSOLIDATION_HOT_TIER_ENABLED", cons_raw.get("hot_tier_enabled", True)
        ),
        warm_tier_enabled=_env_override(
            "CONSOLIDATION_WARM_TIER_ENABLED", cons_raw.get("warm_tier_enabled", True)
        ),
    )
    ...
    dream_loop = DreamLoopConfig(
        ...
        frequency_seconds=_env_override(
            "DREAM_LOOP_FREQUENCY_SECONDS", dream_raw.get("frequency_seconds", 60)
        ),
        batch_size=_env_override(
            "DREAM_LOOP_BATCH_SIZE", dream_raw.get("batch_size", 10)
        ),
        max_iterations=_env_override(
            "DREAM_LOOP_MAX_ITERATIONS", dream_raw.get("max_iterations", 0)
        ),
        subconscious_queue_maxlen=queue_maxlen,
        ollama_url=_env_override(
            "DREAM_LOOP_OLLAMA_URL",
            dream_raw.get("ollama_url", "http://localhost:11434/api/generate"),
        ),
    ...
    synapse = SynapseConfig(
        similarity_threshold=_env_override(
            "SYNAPSE_SIMILARITY_THRESHOLD", syn_raw.get("similarity_threshold", 0.5)
        ),
        auto_bind_on_store=_env_override(
            "SYNAPSE_AUTO_BIND_ON_STORE", syn_raw.get("auto_bind_on_store", True)
        ),
        multi_hop_depth=_env_override(
            "SYNAPSE_MULTI_HOP_DEPTH", syn_raw.get("multi_hop_depth", 2)
        ),
    )
    ...
    context = ContextConfig(
        ...
        shift_threshold=_env_override(
            "CONTEXT_SHIFT_THRESHOLD", ctx_raw.get("shift_threshold", 0.3)
        ),
        rolling_window_size=_env_override(
            "CONTEXT_ROLLING_WINDOW_SIZE", ctx_raw.get("rolling_window_size", 5)
        ),
    )
    ...
    preference = PreferenceConfig(
        ...
        learning_rate=_env_override(
            "PREFERENCE_LEARNING_RATE", pref_raw.get("learning_rate", 0.1)
        ),
        history_limit=_env_override(
            "PREFERENCE_HISTORY_LIMIT", pref_raw.get("history_limit", 100)
        ),
    )
    ...
    anticipatory = AnticipatoryConfig(
        ...
        predictive_depth=_env_override(
            "ANTICIPATORY_PREDICTIVE_DEPTH", ant_raw.get("predictive_depth", 1)
        ),
    )
    ...
    subconscious_ai = SubconsciousAIConfig(
        ...
        beta_mode=_env_override(
            "SUBCONSCIOUS_AI_BETA_MODE", sub_raw.get("beta_mode", True)
        ),
        model_provider=_env_override(
            "SUBCONSCIOUS_AI_MODEL_PROVIDER", sub_raw.get("model_provider", "ollama")
        ),
        model_name=_env_override(
            "SUBCONSCIOUS_AI_MODEL_NAME", sub_raw.get("model_name", "phi3.5:3.8b")
        ),
        model_url=_env_override(
            "SUBCONSCIOUS_AI_MODEL_URL",
            sub_raw.get("model_url", "http://localhost:11434"),
        ),
        api_key=_env_override("SUBCONSCIOUS_AI_API_KEY", sub_raw.get("api_key")),
        api_base_url=_env_override(
            "SUBCONSCIOUS_AI_API_BASE_URL", sub_raw.get("api_base_url")
        ),
        pulse_interval_seconds=_env_override(
            "SUBCONSCIOUS_AI_PULSE_INTERVAL_SECONDS",
            sub_raw.get("pulse_interval_seconds", 120),
        ),
        pulse_backoff_enabled=_env_override(
            "SUBCONSCIOUS_AI_PULSE_BACKOFF_ENABLED",
            sub_raw.get("pulse_backoff_enabled", True),
        ),
        pulse_backoff_max_seconds=_env_override(
            "SUBCONSCIOUS_AI_PULSE_BACKOFF_MAX_SECONDS",
            sub_raw.get("pulse_backoff_max_seconds", 600),
        ),
        max_cpu_percent=_env_override(
            "SUBCONSCIOUS_AI_MAX_CPU_PERCENT", sub_raw.get("max_cpu_percent", 30.0)
        ),
        cycle_timeout_seconds=_env_override(
            "SUBCONSCIOUS_AI_CYCLE_TIMEOUT_SECONDS",
            sub_raw.get("cycle_timeout_seconds", 30),
        ),
        rate_limit_per_hour=_env_override(
            "SUBCONSCIOUS_AI_RATE_LIMIT_PER_HOUR",
            sub_raw.get("rate_limit_per_hour", 50),
        ),
        memory_sorting_enabled=_env_override(
            "SUBCONSCIOUS_AI_MEMORY_SORTING_ENABLED",
            sub_raw.get("memory_sorting_enabled", True),
        ),
        enhanced_dreaming_enabled=_env_override(
            "SUBCONSCIOUS_AI_ENHANCED_DREAMING_ENABLED",
            sub_raw.get("enhanced_dreaming_enabled", True),
        ),
        micro_self_improvement_enabled=_env_override(
            "SUBCONSCIOUS_AI_MICRO_SELF_IMPROVEMENT_ENABLED",
            sub_raw.get("micro_self_improvement_enabled", False),
        ),
        dry_run=_env_override("SUBCONSCIOUS_AI_DRY_RUN", sub_raw.get("dry_run", True)),
        log_all_decisions=_env_override(
            "SUBCONSCIOUS_AI_LOG_ALL_DECISIONS", sub_raw.get("log_all_decisions", True)
        ),
        audit_trail_path=_env_override(
            "SUBCONSCIOUS_AI_AUDIT_TRAIL_PATH",
            sub_raw.get("audit_trail_path", "./data/subconscious_audit.jsonl"),
        ),
        max_memories_per_cycle=_env_override(
            "SUBCONSCIOUS_AI_MAX_MEMORIES_PER_CYCLE",
            sub_raw.get("max_memories_per_cycle", 10),
        ),
    )
    ...
    pulse = PulseConfig(
        ...
        interval_seconds=_env_override(
            "PULSE_INTERVAL_SECONDS", pulse_raw.get("interval_seconds", 30)
        ),
        max_agents_per_tick=_env_override(
            "PULSE_MAX_AGENTS_PER_TICK", pulse_raw.get("max_agents_per_tick", 50)
        ),
        max_episodes_per_tick=_env_override(
            "PULSE_MAX_EPISODES_PER_TICK", pulse_raw.get("max_episodes_per_tick", 200)
        ),
    )
    ...
    performance = PerformanceConfig(
        background_rebuild_enabled=_env_override(
            "PERFORMANCE_BACKGROUND_REBUILD_ENABLED",
            perf_raw.get("background_rebuild_enabled", True),
        ),
        process_priority_low=_env_override(
            "PERFORMANCE_PROCESS_PRIORITY_LOW",
            perf_raw.get("process_priority_low", True),
        ),
        vector_cache_enabled=_env_override(
            "PERFORMANCE_VECTOR_CACHE_ENABLED",
            perf_raw.get("vector_cache_enabled", True),
        ),
        vector_cache_path=_env_override(
            "PERFORMANCE_VECTOR_CACHE_PATH",
            perf_raw.get("vector_cache_path", "./data/vector_cache.sqlite"),
        ),
    )
    ...
    embedding_registry = EmbeddingRegistryConfig(
        enabled=_env_override(
            "EMBEDDING_REGISTRY_ENABLED", embed_raw.get("enabled", True)
        ),
        registry_db_path=_env_override(
            "EMBEDDING_REGISTRY_DB_PATH", embed_raw.get("registry_db_path")
        ),
        auto_migrate=_env_override(
            "EMBEDDING_REGISTRY_AUTO_MIGRATE", embed_raw.get("auto_migrate", False)
        ),
        migration_batch_size=_env_override(
            "EMBEDDING_REGISTRY_MIGRATION_BATCH_SIZE",
            embed_raw.get("migration_batch_size", 100),
        ),
        migration_throttle_ms=_env_override(
            "EMBEDDING_REGISTRY_MIGRATION_THROTTLE_MS",
            embed_raw.get("migration_throttle_ms", 10),
        ),
        max_parallel_workers=_env_override(
            "EMBEDDING_REGISTRY_MAX_PARALLEL_WORKERS",
            embed_raw.get("max_parallel_workers", 2),
        ),
        backup_old_vectors=_env_override(
            "EMBEDDING_REGISTRY_BACKUP_OLD_VECTORS",
            embed_raw.get("backup_old_vectors", True),
        ),
        max_retries=_env_override(
            "EMBEDDING_REGISTRY_MAX_RETRIES", embed_raw.get("max_retries", 3)
        ),
        worker_enabled=_env_override(
            "EMBEDDING_REGISTRY_WORKER_ENABLED", embed_raw.get("worker_enabled", True)
        ),
    )
    ...
    search = SearchConfig(
        ...
        hybrid_alpha=_env_override(
            "SEARCH_HYBRID_ALPHA", search_raw.get("hybrid_alpha", 0.7)
        ),
        rrf_k=_env_override("SEARCH_RRF_K", search_raw.get("rrf_k", 60)),
        sparse_model=_env_override(
            "SEARCH_SPARSE_MODEL", search_raw.get("sparse_model", "bm25")
        ),
        enable_query_expansion=_env_override(
            "SEARCH_ENABLE_QUERY_EXPANSION",
            search_raw.get("enable_query_expansion", True),
        ),
        min_dense_score=_env_override(
            "SEARCH_MIN_DENSE_SCORE", search_raw.get("min_dense_score", 0.0)
        ),
        min_sparse_score=_env_override(
            "SEARCH_MIN_SPARSE_SCORE", search_raw.get("min_sparse_score", 0.0)
        ),
    )
    ...
    dreaming = DreamingConfig(
        ...
        idle_threshold_seconds=_env_override(
            "DREAMING_IDLE_THRESHOLD_SECONDS",
            dream_raw.get("idle_threshold_seconds", 300.0),
        ),
        min_idle_duration=_env_override(
            "DREAMING_MIN_IDLE_DURATION", dream_raw.get("min_idle_duration", 60.0)
        ),
        max_cpu_percent=_env_override(
            "DREAMING_MAX_CPU_PERCENT", dream_raw.get("max_cpu_percent", 25.0)
        ),
        schedules=dream_raw.get(
            "schedules",
            [{"name": "nightly", "cron_expression": "0 2 * * *", "enabled": True}],
        ),
        session_max_duration_seconds=_env_override(
            "DREAMING_SESSION_MAX_DURATION_SECONDS",
            session_raw.get("max_duration_seconds", 600.0),
        ),
        session_max_memories=_env_override(
            "DREAMING_SESSION_MAX_MEMORIES",
            session_raw.get("max_memories_to_process", 1000),
        ),
        enable_episodic_clustering=_env_override(
            "DREAMING_ENABLE_EPISODIC_CLUSTERING",
            session_raw.get("enable_episodic_clustering", True),
        ),
        enable_pattern_extraction=_env_override(
            "DREAMING_ENABLE_PATTERN_EXTRACTION",
            session_raw.get("enable_pattern_extraction", True),
        ),
        enable_recursive_synthesis=_env_override(
            "DREAMING_ENABLE_RECURSIVE_SYNTHESIS",
            session_raw.get("enable_recursive_synthesis", True),
        ),
        enable_contradiction_resolution=_env_override(
            "DREAMING_ENABLE_CONTRADICTION_RESOLUTION",
            session_raw.get("enable_contradiction_resolution", True),
        ),
        enable_semantic_promotion=_env_override(
            "DREAMING_ENABLE_SEMANTIC_PROMOTION",
            session_raw.get("enable_semantic_promotion", True),
        ),
        enable_dream_report=_env_override(
            "DREAMING_ENABLE_DREAM_REPORT", session_raw.get("enable_dream_report", True)
        ),
        cluster_time_window_hours=_env_override(
            "DREAMING_CLUSTER_TIME_WINDOW_HOURS",
            session_raw.get("cluster_time_window_hours", 24.0),
        ),
        pattern_min_frequency=_env_override(
            "DREAMING_PATTERN_MIN_FREQUENCY",
            session_raw.get("pattern_min_frequency", 2),
        ),
        synthesis_max_depth=_env_override(
            "DREAMING_SYNTHESIS_MAX_DEPTH", session_raw.get("synthesis_max_depth", 3)
        ),
        auto_resolve_contradictions=_env_override(
            "DREAMING_AUTO_RESOLVE_CONTRADICTIONS",
            session_raw.get("auto_resolve_contradictions", False),
        ),
        promotion_ltp_threshold=_env_override(
            "DREAMING_PROMOTION_LTP_THRESHOLD",
            session_raw.get("promotion_ltp_threshold", 0.7),
        ),
        persist_reports=_env_override(
            "DREAMING_PERSIST_REPORTS", dream_raw.get("persist_reports", True)
        ),
        report_path=_env_override(
            "DREAMING_REPORT_PATH", dream_raw.get("report_path", "./data/dream_reports")
        ),
        report_include_memory_details=_env_override(
            "DREAMING_REPORT_INCLUDE_MEMORY_DETAILS",
            dream_raw.get("report_include_memory_details", False),
        ),
    )
    ...
    vector_compression = VectorCompressionConfig(
        enabled=_env_override(
            "VECTOR_COMPRESSION_ENABLED", vc_raw.get("enabled", True)
        ),
        pq_n_subvectors=_env_override(
            "VECTOR_COMPRESSION_PQ_N_SUBVECTORS", vc_raw.get("pq_n_subvectors", 32)
        ),
        pq_n_bits=_env_override(
            "VECTOR_COMPRESSION_PQ_N_BITS", vc_raw.get("pq_n_bits", 8)
        ),
        int8_threshold_confidence=_env_override(
            "VECTOR_COMPRESSION_INT8_THRESHOLD_CONFIDENCE",
            vc_raw.get("int8_threshold_confidence", 0.4),
        ),
        age_threshold_hours=_env_override(
            "VECTOR_COMPRESSION_AGE_THRESHOLD_HOURS",
            vc_raw.get("age_threshold_hours", 24.0),
        ),
        compression_interval_seconds=_env_override(
            "VECTOR_COMPRESSION_INTERVAL_SECONDS",
            vc_raw.get("compression_interval_seconds", 3600),
        ),
        max_batch_size=_env_override(
            "VECTOR_COMPRESSION_MAX_BATCH_SIZE", vc_raw.get("max_batch_size", 1000)
        ),
        storage_path=_env_override(
            "VECTOR_COMPRESSION_STORAGE_PATH",
            vc_raw.get("storage_path", "./data/vector_compression.db"),
        ),
        hot_tier_compression=_env_override(
            "VECTOR_COMPRESSION_HOT_TIER", vc_raw.get("hot_tier_compression", False)
        ),
        warm_tier_compression=_env_override(
            "VECTOR_COMPRESSION_WARM_TIER", vc_raw.get("warm_tier_compression", True)
        ),
        cold_tier_compression=_env_override(
            "VECTOR_COMPRESSION_COLD_TIER", vc_raw.get("cold_tier_compression", True)
        ),
    )
    ...
    backup = BackupConfig(
        ...
        auto_snapshot_enabled=_env_override(
            "BACKUP_AUTO_SNAPSHOT_ENABLED",
            backup_raw.get("auto_snapshot_enabled", True),
        ),
        snapshot_interval_hours=_env_override(
            "BACKUP_SNAPSHOT_INTERVAL_HOURS",
            backup_raw.get("snapshot_interval_hours", 24),
        ),
        max_snapshots=_env_override(
            "BACKUP_MAX_SNAPSHOTS", backup_raw.get("max_snapshots", 7)
        ),
        compression_enabled=_env_override(
            "BACKUP_COMPRESSION_ENABLED", backup_raw.get("compression_enabled", True)
        ),
        wal_enabled=_env_override(
            "BACKUP_WAL_ENABLED", backup_raw.get("wal_enabled", True)
        ),
        wal_flush_interval_seconds=_env_override(
            "BACKUP_WAL_FLUSH_INTERVAL_SECONDS",
            backup_raw.get("wal_flush_interval_seconds", 300),
        ),
        wal_max_size_mb=_env_override(
            "BACKUP_WAL_MAX_SIZE_MB", backup_raw.get("wal_max_size_mb", 100)
        ),
        backup_dir=_env_override(
            "BACKUP_DIR", backup_raw.get("backup_dir", "./backups")
        ),
        verify_checksums=_env_override(
            "BACKUP_VERIFY_CHECKSUMS", backup_raw.get("verify_checksums", True)
        ),
        retention_days=_env_override(
            "BACKUP_RETENTION_DAYS", backup_raw.get("retention_days", 30)
        ),
        keep_daily=_env_override("BACKUP_KEEP_DAILY", backup_raw.get("keep_daily", 7)),
        keep_weekly=_env_override(
            "BACKUP_KEEP_WEEKLY", backup_raw.get("keep_weekly", 4)
        ),
        keep_monthly=_env_override(
            "BACKUP_KEEP_MONTHLY", backup_raw.get("keep_monthly", 12)
        ),
        restore_timeout_seconds=_env_override(
            "BACKUP_RESTORE_TIMEOUT_SECONDS",
            backup_raw.get("restore_timeout_seconds", 300),
        ),
        verify_after_restore=_env_override(
            "BACKUP_VERIFY_AFTER_RESTORE", backup_raw.get("verify_after_restore", True)
        ),
    )
    ...
    eft = EFTConfig(
        ...
        max_scenarios_per_simulation=_env_override(
            "EFT_MAX_SCENARIOS", eft_raw.get("max_scenarios_per_simulation", 5)
        ),
        min_similarity_threshold=_env_override(
            "EFT_MIN_SIMILARITY", eft_raw.get("min_similarity_threshold", 0.55)
        ),
        temporal_horizon_hours=_env_override(
            "EFT_TEMPORAL_HORIZON", eft_raw.get("temporal_horizon_hours", 24.0)
        ),
        branching_factor=_env_override(
            "EFT_BRANCHING_FACTOR", eft_raw.get("branching_factor", 3)
        ),
        scenario_decay_lambda=_env_override(
            "EFT_DECAY_LAMBDA", eft_raw.get("scenario_decay_lambda", 0.05)
        ),
        scenario_half_life_hours=_env_override(
            "EFT_HALF_LIFE", eft_raw.get("scenario_half_life_hours", 12.0)
        ),
        min_scenario_confidence=_env_override(
            "EFT_MIN_CONFIDENCE", eft_raw.get("min_scenario_confidence", 0.1)
        ),
        attention_boost_factor=_env_override(
            "EFT_ATTENTION_BOOST", eft_raw.get("attention_boost_factor", 0.2)
        ),
        scenario_attention_weight=_env_override(
            "EFT_ATTENTION_WEIGHT", eft_raw.get("scenario_attention_weight", 0.15)
        ),
        max_stored_scenarios=_env_override(
            "EFT_MAX_STORED", eft_raw.get("max_stored_scenarios", 100)
        ),
        persist_scenarios=_env_override(
            "EFT_PERSIST", eft_raw.get("persist_scenarios", True)
        ),
    )
    ...
    working_memory_cfg = WorkingMemoryConfig(
        max_items_per_agent=_env_override(
            "WM_MAX_ITEMS", wm_raw.get("max_items_per_agent", 20)
        ),
        default_ttl_seconds=_env_override(
            "WM_DEFAULT_TTL", wm_raw.get("default_ttl_seconds", 3600)
        ),
    ...
    episodic_cfg = EpisodicConfig(
        ...
        tier_persistence_enabled=_env_override(
            "EPISODIC_TIER_PERSISTENCE", ep_raw.get("tier_persistence_enabled", True)
        ),
    )
    ...
    semantic_cfg = SemanticConfig(
        ...
        enable_qdrant_persistence=_env_override(
            "SEMANTIC_QDRANT_ENABLED", sem_raw.get("enable_qdrant_persistence", True)
        ),
        auto_consolidate_from_episodes=sem_raw.get(
            "auto_consolidate_from_episodes", True
        ),
    ...
    procedural_cfg = ProceduralConfig(
        ...
        reliability_penalty_on_failure=proc_raw.get(
            "reliability_penalty_on_failure", 0.10
        ),
    ...
    meta_memory_cfg = MetaMemoryConfig(
        ...
        anomaly_failure_rate_threshold=mm_raw.get(
            "anomaly_failure_rate_threshold", 0.10
        ),
        ...
        enable_llm_proposals=_env_override(
            "META_MEMORY_LLM_PROPOSALS", mm_raw.get("enable_llm_proposals", True)
        ),
    )
    ...
    self_improvement_cfg = SelfImprovementConfig(
        ...
        allow_llm_rewrite=_env_override(
            "SELF_IMPROVEMENT_LLM_REWRITE", si_raw.get("allow_llm_rewrite", False)
        ),
    )
    ...
    associations_cfg = AssociationsConfig(
        auto_save=_env_override(
            "ASSOCIATIONS_AUTO_SAVE", assoc_raw.get("auto_save", True)
        ),
        decay_enabled=_env_override(
            "ASSOCIATIONS_DECAY_ENABLED", assoc_raw.get("decay_enabled", True)
        ),
    )
