...
class CognitiveRouter:
    ...
    def _assess_complexity(self, text: str) -> float:
        ...
        # Length heuristic
        if len(text.split()) > 20: score += 0.3
    ...
    def _system_2_reasoning(self, impulse: str, context: Optional[dict]) -> str:
        ...
        if self.engine.epistemic_drive_active:
            ...
            # If the caller provides a working-memory context...
            ctx_vec: Union[HDV, BinaryHDV]
            ...
            if context and isinstance(context.get("working_memory"), list) and context["working_memory"]:
                ...
                # Check if we are in Binary or Float mode
                if vectors and isinstance(vectors[0], BinaryHDV):
                    # Filter out non-BinaryHDVs just in case
                    bin_vecs = [v for v in vectors if isinstance(v, BinaryHDV)]
                    if bin_vecs:
                    ...
                    for v in vectors:
                        if isinstance(v, HDV):
                            combined += v.vector
                    ctx_vec = HDV(dimension=self.engine.dimension)
                    ctx_vec.vector = np.sign(combined)
